...
// ---------------------------------------------------------------------------
// renew_token_now / daemon lifecycle
// ---------------------------------------------------------------------------
...
async fn renew_token_now_propagates_api_error() {
    ...
    let client = build_test_client(&server).await;
    // A 500 must surface as an error, guarding against renew_token_now being
...
async fn lease_watcher_try_recv_surfaces_queued_event() {
    ...
    // guarding against try_recv being replaced with an unconditional None
